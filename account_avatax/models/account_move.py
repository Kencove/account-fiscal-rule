...
from odoo.addons.account.models.account_move import AccountMove as AccountMoveClass
...
def _recompute_tax_lines_new(  # noqa: C901
    ...
    def _serialize_tax_grouping_key(grouping_dict):
        ...
        '_get_tax_grouping_key_from_tax_line' or '_get_tax_grouping_key_from_base_line'.
        :return: A string representing the values.
        """
        return "-".join(str(v) for v in grouping_dict.values())

    def _compute_base_line_taxes(base_line):
        """Compute taxes amounts both in
        company currency / foreign currency as the ratio between
        amount_currency & balance could not be
        the same as the expected currency rate.
        The 'amount_currency' value
        will be set on compute_all(...)['taxes'] in multi-currency.
        ...
        balance_taxes_res = base_line.tax_ids._origin.with_context(
            force_sign=move._get_tax_force_sign(), base_line=base_line
        ).compute_all(
            price_unit_wo_discount,
...
AccountMoveClass._recompute_tax_lines = _recompute_tax_lines_new
